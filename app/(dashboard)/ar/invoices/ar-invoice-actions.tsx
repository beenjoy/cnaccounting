...
export function ARInvoiceActions({ companyId, customers, mode, invoice }: Props) {
  ...
  async function handleCancel() {
    setLoading(true);
    try {
      const res = await fetch(`/api/ar-invoices/${invoice!.id}`, { method: "DELETE" });
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-lg shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            <div className="px-6 py-4 border-b">
              <h2 className="text-lg font-semibold">新建应收发票</h2>
            </div>
            <form onSubmit={handleSubmit} className="px-6 py-4 space-y-4">
              <div>
                <label className="block text-sm font-medium mb-1">客户 <span className="text-red-500">*</span></label>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input value={form.invoiceNumber} onChange={(e) => setForm({ ...form, invoiceNumber: e.target.value })}
                    required className="w-full rounded-md border px-3 py-1.5 text-sm" placeholder="如：INV-2026-001" />
                </div>
                <div>

...
import { ARInvoiceActions } from "./ar-invoice-actions";
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">应收发票</h1>
        ...
        {canEdit && <ARInvoiceActions companyId={company.id} customers={customers} mode="new" />}
      </div>

      {/* 筛选 */}
      <div className="flex items-center gap-3">
        <form method="GET" action="/ar/invoices" className="flex items-center gap-2">
          ...
          {(sp.customerId || sp.status) && (
            <Link href="/ar/invoices" className="text-sm text-muted-foreground hover:underline">清除</Link>
          )}
        </form>
      </div>
          ...
          <table className="w-full text-sm">
            <thead className="bg-muted/40 border-b">
              <tr>
                ...
                <th className="px-4 py-3 text-right font-medium text-muted-foreground">已收款</th>
                <th className="px-4 py-3 text-right font-medium text-muted-foreground">未收款</th>
